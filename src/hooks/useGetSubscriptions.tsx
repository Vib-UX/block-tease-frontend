...
const useGetSubscriptions = () => {
  ...
  return useQuery([email, 'subscription'], async (): Promise<CardData[]> => {
    ...
    const goodData = filterMatchingIds(
      jsonData.data.subscriptions,
      allModelData
    );
    const data = await Promise.all(
      goodData.map(async (card: CardData) => {
        const ipfsData = { name: card.modelName, image: card.ipfsUrl };
