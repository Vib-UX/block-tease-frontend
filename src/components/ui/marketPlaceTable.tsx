...
function createData(
  ...
  return { icon, name, floor, floor1d, volume, TopOffer, Sales, Listed, id, tokenId };
}
...
export default function CustomizedTables() {
  ...
  const handleBuyNft = async (tokenId: any) => {
    BuyNft(provider, tokenId);
  };
  return (
    <TableContainer
      component={Paper}
          ...
          {rows.map((row) => (
            <StyledTableRow key={row.name}>
              <StyledTableCell component='th' scope='row'>
                <Image src={row.icon} className='w-10 h-10 rounded-full' alt='modelIcon' />
              </StyledTableCell>
              <StyledTableCell component='th' scope='row'>
                <div className='flex items-center  gap-2 '>
                  {row.name}
                  <svg onClick={() => handleBuyNft(row.tokenId)} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="cursor-pointer hover:text-fuchsia-600 w-5 h-5">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v6m3-3H9m12 0a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />
                  </svg>
                </div>
              </StyledTableCell>

              <StyledTableCell align='right'>{row.floor}<span className='text-slate-400'>USDC</span></StyledTableCell>
              ...
              <StyledTableCell align='right'>{row.volume}<span className='text-slate-400'>USDC</span></StyledTableCell>
              <StyledTableCell align='right'>
                {row.TopOffer} USDC
              </StyledTableCell>
