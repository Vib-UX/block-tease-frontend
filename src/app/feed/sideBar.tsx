          ...
          {sideBarContent.map((item, index) => (
            <div key={index} className='flex gap-3 items-center '>
              <Image
                ...
                height={20}
              />
              <p className='hidden lg:block '>{item.tag}</p>
            </div>
          ))}
        </div>
        <p className='text-[#FB0393] font-bold text-xl '>Categories</p>
      </div>
      <div className='flex gap-2 w-full items-center justify-center lg:justify-start  h-[30%] '>
        <svg
